...
export async function logEvents(request, env, { ip, askGuard, json, refuse }) {
  ...
  const insert = env.DB.prepare(
    'INSERT INTO events (day, match_id, research, lang, app_ver, game, game_ver, mode, kind, actor, model, phase, act, detail) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
  const bump = env.DB.prepare(
    'INSERT INTO agg (game, game_ver, mode, actor, model, kind, phase, act, detail, n) VALUES (?,?,?,?,?,?,?,?,?,?) ' +
    'ON CONFLICT (game, game_ver, mode, actor, model, kind, phase, act, detail) DO UPDATE SET n = n + excluded.n');

  const counts = new Map();
  const stmts = batch.e.map((e) => {
    const key = [e.g, e.gv, e.m, e.a || '', e.mdl || '', e.k, e.ph || '', e.act, e.x || ''];
    const k = key.join('|');
    counts.set(k, { key, n: (counts.get(k)?.n || 0) + 1 });
    return insert.bind(day, batch.s, batch.r ? 1 : 0, batch.l, batch.av, e.g, e.gv, e.m, e.k, e.a ?? null, e.mdl ?? null, e.ph ?? null, e.act, e.x ?? null);
  });
...
export async function getStats(env, ctx, cors) {
  ...
  const cacheKey = new Request('https://stats.cache/v1');
  const hit = cache && await cache.match(cacheKey);
  let body = hit ? await hit.text() : null;
  if (!body) {
    const { results } = await env.DB.prepare(
      'SELECT game, game_ver, mode, actor, model, kind, phase, act, detail, n FROM agg WHERE n > 0').all();
    body = JSON.stringify({
      enabled: true,
      updated: new Date().toISOString(),
      cols: ['game', 'game_ver', 'mode', 'actor', 'model', 'kind', 'phase', 'act', 'detail', 'n'],
      rows: results.map((r) => [r.game, r.game_ver, r.mode, r.actor, r.model, r.kind, r.phase, r.act, r.detail, r.n]),
    });
